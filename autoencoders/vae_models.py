...
class VAE(AE):
    ...
    def latent_dist(self, loc, logscale):
        return self.latent_dist_(loc=loc, scale=tf.exp(logscale))
    
    def prior_dist(self, loc, logscale):
        return self.latent_dist_(loc=loc, scale=tf.exp(logscale))
    
    def decoder_dist(self, loc, logscale=None, eps_scale=0.1):
    ...
    def __call__(self, x, **decoder_kwargs):
        loc, logscale = self.encode(x)
        latent_dist = self.latent_dist(loc, logscale)
        z = latent_dist.sample()
        decoded = self.decode(z, **decoder_kwargs)
...
class GaussianVAE(VAE):
    ...
    def kl_loss(self, model_outputs):
        ...
        prior = self.prior_dist(tf.zeros_like(posterior.loc), tf.zeros_like(posterior.scale))
...
class CauchyVAE(VAE):
    def __init__(self, *args, 
                 ...
                 decoder_dist=tfd.Cauchy, **kwargs):
